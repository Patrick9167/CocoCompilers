...
out type);
	...
	void Stat() {
		int type; Op op; string name; Obj obj; int reg, regT;
		switch (la.kind) {
		case 2: {
			Ident(out name);
			obj = tab.Find(name); 
...
out type);
	...
	static readonly bool[,] set = {
		...
		{x,x,T,x, T,T,T,T, x,x,x,T, T,T,T,T, T,x,T,x, T,T,T,T, T,T,x,x, x,x,T,x, T,T,x,T, x,T,T,x, x,x,T,T, T,T,x,x, x,x,x,x, x},
